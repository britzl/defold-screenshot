...
static void DumpPixels(GLubyte* pixels, unsigned int from, unsigned int to) {
  for (unsigned int i=from; i < to; i++) {
    printf("%i R:%02x G:%02x B:%02x A:%02x\n", i, pixels[0 + (i * 4)], pixels[1 + (i * 4)], pixels[2 + (i * 4)], pixels[3 + (i * 4)]);
  }
}
